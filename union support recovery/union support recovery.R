...
n = 200                            #data size
p = 600                              #number of total predictors
q = p^(-1)
p1 = p
...
rd3 = rd[c(-3,-4)]
rd4 = rd[c(-4,-5)]
union.model = true.model1 =true.model2 =true.model3 =true.model4 = rep(0,p)
true.model1[rd1]=1
true.model2[rd2]=1
true.model3[rd3]=1
true.model4[rd4]=1
...
for(i in 1:sn){
  sgn = rbinom(1,1,0.5)
  beta1_0[rd1[i]] = (-1)^sgn*runif(1,0.5,1)*2
  ...
  #   beta1_0[i] = beta1_0[i] - 100
  # }
  if(abs(beta1_0[rd1[i]])<1/n){i = i-1}
}
...
for(i in 1:sn){
  sgn = rbinom(1,1,0.5)
  beta2_0[rd2[i]] = (-1)^sgn*runif(1,0.5,1)*2
  ...
  #   beta2_0[i] = beta2_0[i] - 100
  # }
  if(abs(beta2_0[rd2[i]])<1/n){i = i-1}
}
...
for(i in 1:sn){
  sgn = rbinom(1,1,0.5)
  beta3_0[rd3[i]] = (-1)^sgn*runif(1,0.5,1)*2
  ...
  #   beta2_0[i] = beta2_0[i] - 100
  # }
  if(abs(beta3_0[rd3[i]])<1/n){i = i-1}
}
...
for(i in 1:sn){
  sgn = rbinom(1,1,0.5)
  beta4_0[rd4[i]] = (-1)^sgn*runif(1,0.5,1)*2
  ...
  #   beta2_0[i] = beta2_0[i] - 100
  # }
  if(abs(beta4_0[rd4[i]])<1/n){i = i-1}
}
...
for(iter in 1:N){
  ...
  y = rmvt(n, sigma=sig_cor, df=degree)+ cbind(x1%*%beta1_0,x2%*%beta2_0,x3%*%beta3_0,x4%*%beta4_0)
  y1 = y[,1]
  y2 = y[,2]
  y3 = y[,3]
  y4 = y[,4]
